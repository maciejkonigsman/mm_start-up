...
function fpUploadForm($headerMessage = '') {
	...
	<table class="form-table"><tbody>
		...
		<tr class="form-field form-required">
			...
			<td class="field"><input name="grabfrom_saveas" type="text" required value="' . (isset($_POST['grabfrom_saveas']) ? $_POST['grabfrom_saveas'] : '') . '"></td>
		</tr>
	</table>';
	...
	function validateGrabfromFrom() {
		...
		var url = jQuery("input[name='grabfrom_url']").val() || '',
			...
			saveasExt = (saveas.lastIndexOf('.') == -1) ? '' : saveas.substring(saveas.lastIndexOf('.')+1).toLowerCase();
		// fix: allow images to contain query params, like test.png?w=1090
		if (urlExt.indexOf('?') >= 0) {
			urlExt = urlExt.substr(0, urlExt.indexOf('?'));
		}
		// fix: if no file extension supplied, use original file ext
		if (saveasExt == '') {
			saveasExt = urlExt;
			saveas = saveas + '.' +  urlExt;
			jQuery("input[name='grabfrom_saveas']").val(saveas);
		}
		...
		if (!isValid(saveas)) {
			alert('Save As file name is not valid! Please use only valid characters.');
			return(false);
		}
		if (saveasExt != urlExt) {
